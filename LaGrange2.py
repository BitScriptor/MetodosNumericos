...
l = 1 #Declaracion de nuevas variables
y =0
...
for i in range(len(ejex)):
	...
	for j in range(len(ejex)):  #ciclos for anidados para poder aplicar la formula.
		...
		if(j != i): #condicion para que L0 no tome en cuenta cuando j == i
			l *= ((x -ejex[j]) / (ejex[i] - ejex[j]))
			
		else:
			j += 1
			
	print "\n\t\tL%d = " %i, l # impresion de las Formulas obtenidas
	
	y += (l * ejey[i]) #sumatoria de del polinomio de LaGrange
	y1 = simplify(y)
	
	l=1 #Reseteo de la variable para la siguiente iteracion
	
print "\n\nP(x) = ", y1 #Impresion de P(x)
